...
Что делает:
1. Патчит `openclaw.json` — добавляет агента researcher, разрешения, лимиты, запись плагина
2. Создаёт workspace researcher-а — папку с промптом субагента (`AGENTS.md`, `TOOLS.md`)
3. Пересобирает `AGENTS.md` из markdown-блоков и текущего plugin config
4. Создаёт `INDEX.md` — пустой реестр исследований
5. Coordinator skill грузится нативно из `skills/deep-research/SKILL.md` через `openclaw.plugin.json`
...
`setup` идемпотентен — безопасно запускать повторно. Он пересобирает `AGENTS.md` из блоков, проверяет и дополняет конфиг, не трогая пользовательские tool toggles. После — рестарт Gateway.
...
Промпт собирается из блоков во время `openclaw deep-research setup`. Пользователь включает/выключает инструменты в конфиге плагина, затем повторно запускает `setup`, и команда пересобирает `workspace-researcher/AGENTS.md` только из включённых блоков.
...
`cascades.md` — шаблон с деревьями решений. Сборщик промпта вырезает строки, содержащие упоминания выключенных инструментов. Каскады удобнее держать в файле, а не генерировать кодом — проще читать и редактировать.
