...
Для простых случаев — когда нужен контент без сложной логики.
...
Полноценный облачный браузер с `render_js: true`. Обходит anti-bot, работает через гео-прокси (residential IP из любой страны). Поддерживает многошаговые сценарии (заполнение форм, клики, скролл).
...
- `render_js` — запустить облачный браузер (boolean, default: true)
