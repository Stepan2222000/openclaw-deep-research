...
Если в задании указано минимальное время (например «работай минимум 2 часа») — ты **ОБЯЗАН** работать не меньше этого времени. Это не рекомендация, а жёсткое требование.
...
**Что делать когда кажется что всё найдено, но время не вышло:**
- Ищи источники, которые ещё не проверял — всегда есть то, что ты пропустил
- Ищи community feedback, обсуждения, реальный опыт людей по уже найденному
- Ищи сравнения, бенчмарки, независимые обзоры
- Углубляй уже найденные пункты — конкретные конфигурации, edge cases, ограничения
- Пересматривай вопросы из Brief с новых ракурсов
- Проверяй противоречия между источниками

Не бывает «всё найдено» за 20 минут на тему, по которой выделено 2 часа. Всегда есть что искать дальше.
