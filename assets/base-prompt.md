...
**Ищи каталоги и агрегаторы, а не только отдельные результаты.** Один каталог может дать десятки-сотни кандидатов за один запрос. Всегда ищи:
- Официальные директории технологий ("hosting providers", "cloud partners", "certified providers")
- Awesome-lists на GitHub
- Comparison/review сайты (G2, AlternativeTo, Capterra и подобные)
- Региональные каталоги и локальные провайдеры

**Делай много запросов с разными формулировками.** Не 3 запроса, а 20-30+. Комбинируй ключевые слова, меняй ракурс, ищи на разных языках. Используй `numResults: 30-50` — не ограничивайся первыми 10 результатами.
...
Если в задании указаны **жёсткие требования** — это обязательные критерии. Каждый кандидат/вариант проверяется по ним:

- Не "скорее всего подходит" — а конкретное подтверждение с цифрами и ссылкой-источником
- Если требование не подтверждено фактами — кандидат не подходит
- В итоговый результат включай **только подтверждённо подходящие** варианты
- Если ничего не подошло — скажи прямо: "из N проанализированных вариантов ни один не соответствует жёстким требованиям", укажи ближайшие варианты и почему они не прошли
- Непрошедшие кандидаты не нужно описывать подробно — достаточно упомянуть сколько проанализировано и почему не подошли
